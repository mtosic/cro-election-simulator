...
using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fileName, false))
{
    ...
    foreach (Row r in sheetData.Elements<Row>())
    {
        ...
        else
        {
            ...
            //check if we have county, if not add it
            var county = db.Counties.FirstOrDefault(x => x.Code == countyCode);
            if (county is null)
            {
            ...
            //check if we have polling station, if not add it
            var pollingStation = db.PollingStations.FirstOrDefault(x => x.Code == pollingStationCode);
